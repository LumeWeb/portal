...
func (cs *ConfigServiceDefault) handleConfigChange(key string, value any) error {
	...
	switch category {
	case config.CONFIG_PROPERTY_UPDATE_EVENT_CATEGORY_CORE:
		err := cs.config.FieldProcessor(cs.config.Config(), "", cs.configUpdatesProcessor)
		if err != nil {
			return err
		}
	case config.CONFIG_PROPERTY_UPDATE_EVENT_CATEGORY_SERVICE:
	...
	case config.CONFIG_PROPERTY_UPDATE_EVENT_CATEGORY_PROTOCOL:
	...
	case config.CONFIG_PROPERTY_UPDATE_EVENT_CATEGORY_API:
