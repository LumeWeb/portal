...
//     Recommended way to mock the configuration returned by `ctx.GetConfig().GetAPI(apiID)`.
...
//     Recommended way to mock the configuration returned by `ctx.GetConfig().GetProtocol(protocolID)`.
//     Sets a `Maybe()` expectation on the mock `ConfigManager`. Include this option
...
//     Recommended way to mock the configuration returned by `ctx.GetConfig().GetService(serviceID)`.
//     Sets a `Maybe()` expectation on the mock `ConfigManager`. Include this option
...
//
//		coreTesting.RunTestCase(t, func(tb coreTesting.TB, ctx coreTesting.TestContext) {
//			// When BootEnvironment runs, if MyService calls ctx.GetConfig().GetService("myservice"),
...
//    Use `ctx.GetConfig().GetConfig().Core.Domain` for the main domain or construct
...
//			fullDomain := fmt.Sprintf("%s.%s", apiSubdomain, ctx.GetConfig().GetConfig().Core.Domain)
//			req.Host = fullDomain
//
...
// 9.  **API Route Testing:** When testing HTTP routes, use `ctx.Router()`, `net/http.NewRequest`, and `httptest.NewRecorder`. Always set the `req.Host` header correctly, especially for subdomain-based APIs, using `ctx.GetConfig().GetConfig().Core.Domain`.
